...
unsigned char HID_ReportDescriptor[53] = {
    ...
    0x95, 0x18,    //46      GLOBAL_REPORT_COUNT(24)  
...
static ssize_t insert_report_descriptor(struct f_hid_opts *opts)
{
  ...
  opts->subclass = 0x00; 
  opts->protocol = 0x00;
	opts->report_desc = HID_ReportDescriptor;
	opts->report_desc_length = 0x0035;
	opts->report_length = 0x0004;
...
static int hidg_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (hidg->in_ep != NULL) {
		...
		memset(hidg->req->buf, 0, hidg->report_length);
